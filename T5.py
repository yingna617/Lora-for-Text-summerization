...
Original file is located at
    https://colab.research.google.com/drive/1O4jKk3P74R0F6vPQ12yYmwhE60NLzNKz
...
train = dict(x_train,y_train)
validation =dict(x_valid,y_valid)
test = dict(x_test, y_test)
...
ratio=1
train = split(train,ratio)
validation= split(validation,ratio)
test= split(test,0.05)
...
output_dir = f't5-lora-ar-training-{str(int(time.time()))}'
...
# push to Hub
peft_model.push_to_hub("t5-lora-100-final")

import shutil

# 模型目录在 Colab 中的路径
colab_model_dir = '/content/t5-lora-ar'
# 模型目录要保存到 Google Drive 中的路径
drive_model_dir = '/content/drive/My Drive/t5-lora-ar-final'
...
# 加载模型
new_model = AutoModelForSeq2SeqLM.from_pretrained('./t5-lora-ar')
new_model.to(device)

new_model.push_to_hub("t5-lora-ar")

# Load model directly
from transformers import AutoModelForSeq2SeqLM
model = AutoModelForSeq2SeqLM.from_pretrained('/content/drive/MyDrive/t5-lora-ar-final')
model.to(device)
...
def example(text):
  ...
  peft_model_outputs = model.generate(input_ids=input_ids, max_length=64, )
